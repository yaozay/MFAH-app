      ...
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {(plans ?? []).map((plan) => {
              const finalPrice =
                (Number(plan.price ?? 0) - Number(plan.discount_amt ?? 0)).toFixed(2);

              const isTopPlan = Boolean(plan.is_top);
                    ...
                    {!manage &&
                      (!user ||
                        ...
                        <button
                          ...
                          className={`w-full py-3 font-medium rounded-lg transition-all ${isTopPlan
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <LabeledInput
                ...
                step="0.01"
                value={editing?.price ?? ""}
                onChange={(v) => setEditing((s) => ({ ...s, price: v }))}
              />
              <LabeledInput
                label="Discount"
                type="number"
                step="0.01"
                value={editing?.discount_amt ?? 0}
                onChange={(v) => setEditing((s) => ({ ...s, discount_amt: v }))}
              />
              <LabeledInput
                label="Duration (months)"
                type="number"
                value={editing?.duration_months ?? 12}
                onChange={(v) => setEditing((s) => ({ ...s, duration_months: v }))}
              />
              <LabeledInput
                label="People Included"
                type="number"
                value={editing?.people_included ?? 1}
                onChange={(v) => setEditing((s) => ({ ...s, people_included: v }))}
              />
              <LabeledInput
                label="Display Order"
                type="number"
