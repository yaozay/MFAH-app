...
export default function ExhibitionsForm() {
  ...
  const [venues, setVenues] = useState([]); //   Holds venue list for dropdown
  ...
  //   Load exhibitions and venues on mount
  useEffect(() => {
  ...
  async function handleDelete(id) {
    ...
    try {
      await fetch(`${API}/api/exhibitions/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      setExhibitions(exhibitions.filter((e) => e.exhibition_id !== id));
    } catch (err) {
